...
export function ReviewTable({ rows, selected, onToggle, onSelectAll }: ReviewTableProps) {
  const exportable = rows.filter((row) => row.event);
  const allSelected =
          ...
          <tbody>
            {rows.map((row) => {
              ...
              const issue =
                ...
                row.status === 'Hibás párosítás' ||
                row.status === 'Sikertelen';
              return (
                <tr
                  key={row.id}
                  className={issue ? 'row-issue' : row.status === 'Kizárva' ? 'row-muted' : ''}
                >
                  ...
                  <td data-label="Kezdés">{time(row.start)}</td>
                  <td data-label="Befejezés">{time(row.end)}</td>
                  <td data-label="Esemény">{row.summary ?? '—'}</td>
                  <td data-label="Állapot">
                    <span className={`status status-${issue ? 'issue' : 'ok'}`}>{row.status}</span>
                  </td>
